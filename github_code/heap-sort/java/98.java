 ...
 * <ul>
 * <li>The top N entries can be sorted in ascending order, so that the N
 * elements with the highest values are placed in the array's last N positions
 * (the highest entry of the arrays will be in array[array.length-1], the second
 * highest in array[array.length-2] and so on.</li>
 * <li>The top N entries can be sorted in descending order, so that the N
 * elements with the highest values are placed in the array's first N positions
 * (the highest entry of the array will be in array[0], the second highest entry
 * will be in array[1] and so on.</li>
 * </ul>
 * 
...
public class HeapSortInt
{
	 ...
	 * @param C
	 *            int[] an additional array to sort by A. based on the values of
	 *            the first argument.
	 */
	 ...
	 * 
	 * @param A
	 *            int[] the first array to be sorted.
	 * @param B
	 *            int[] the second array to be sorted, according to the values
	 *            of the first array.
	 */
	 ...
	 * @param C
	 *            int[] an additional array to sort by A. values of the first
	 *            array.
	 */
	 ...
	 */
	/*
	 * private int buildMaxHeap(int[] A, int[] B, int[] C) { int heapSize =
	 * A.length; for (int i = (int) Math.floor(heapSize / 2.0D); i > 0; i--)
	 * maxHeapify(A, B, C, i, heapSize); return heapSize; }
	 */
	 ...
	 */
	/*
	 * public void heapSort(int[] A, int[] B, int[] C) { int heapSize =
	 * buildMaxHeap(A, B, C); int tmpDouble; int tmpInt; final int Alength =
	 * A.length; for (int i = Alength; i > 0; i--) { tmpDouble = A[i - 1]; A[i -
	 * 1] = A[0]; A[0] = tmpDouble; tmpInt = B[i - 1]; B[i - 1] = B[0]; B[0] =
	 * tmpInt; tmpInt = C[i - 1]; C[i - 1] = C[0]; C[0] = tmpInt; heapSize--;
	 * maxHeapify(A, B, C, 1, heapSize); } } /** Sorts the top
	 * <tt>topElements</tt> of the given array in ascending order using heap
	 * sort.
	 * 
	 ...
	 */
	/*
	 * public void heapSort(int[] A, int[] B, int[] C, int topElements) { int
	 * heapSize = buildMaxHeap(A, B, C);
	 * 
	 * final int Alength = A.length; final int end = Alength - topElements; int
	 * tmpDouble; int tmpInt; for (int i = Alength; i > end; i--) { tmpDouble =
	 * A[i - 1]; A[i - 1] = A[0]; A[0] = tmpDouble; tmpInt = B[i - 1]; B[i - 1]
	 * = B[0]; B[0] = tmpInt;
	 * 
	 * tmpInt = C[i - 1]; C[i - 1] = C[0]; C[0] = tmpInt;
	 * 
	 * heapSize--; maxHeapify(A, B, C, 1, heapSize); } } /** Maintains the heap
	 * property.
	 * 
	 * @param A int[] The array on which we operate.
	 * 
	 * @param i int a position in the array. This number is between 1 and
	 * A.length inclusive.
	 */

	/*
	 * private void maxHeapify(int[] A, int[] B, int[] C, int i, int heapSize) {
	 * int largest; int l = 2 * i; int r = 2 * i + 1; if (l <= heapSize && A[l -
	 * 1] > A[i - 1]) largest = l; else largest = i; if (r <= heapSize && A[r -
	 * 1] > A[largest - 1]) largest = r; if (largest != i) { int tmpDouble =
	 * A[largest - 1]; A[largest - 1] = A[i - 1]; A[i - 1] = tmpDouble; int
	 * tmpInt = B[largest - 1]; B[largest - 1] = B[i - 1]; B[i - 1] = tmpInt;
	 * 
	 * tmpInt = C[largest - 1]; C[largest - 1] = C[i - 1]; C[i - 1] = tmpInt;
	 * 
	 * 
	 * maxHeapify(A, B, C, largest, heapSize); } }
	 */
