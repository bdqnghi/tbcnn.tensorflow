...
vector<Student> readinput(char * fName){  
	...
	if (in.is_open()) { 
		while (!in.eof()) { 
			...
			trim(word);  
			str.set(word); 
			stu.set_name(trim(name = str.next())); 
			stu.set_telno(trim(telno = str.next())); 
			words.push_back(stu); 
		}
	}
	return words;
...
void partition(int low, int high, int& pivotpoint, vector<Student>& A){ 
	int i;
	int j;
...
void quicksort(int low, int high, vector<Student> &A){ 
	int pivotpoint;
	if( high > low){
