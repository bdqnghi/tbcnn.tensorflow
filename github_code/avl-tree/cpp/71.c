  ...
  std::string sorted_filename, std::string sorted_filename2)
{
 ...
 /* std::cout << "\n After sorting - grader::\n";
  for (int i = 0; i < product_points.size(); i++)
  {
	  std::cout << "(" << product_points[i]->getX() << "," << product_points[i]->getY() << ")" << "\t";
  }*/
...
TestCase * Grader04::testBTree(std::string cmds_filename)
{
  ...
  for(size_t i = 0; i < commands.size(); ++i){
    ...
    if(cmd.action == BTREE_CMD_INSERT){
      ...
      inorderTraversal(rb_tree, rb_sorted);
	  std::cout << "rb_sorted:\n";
	  for (int i = 0; i < rb_sorted.size(); i++)
	  {
		  std::cout << rb_sorted[i] << "\t";
	  }
	  ...
	  std::cout << "no of keys in the root "<<root->getKeySize() << "\n";
	  for (int i = 0; i < root->getKeySize(); i++)
		  std::cout << "root->getkey " << root->getKey(i) << "\t";

	  std::cout << "\n\n";

      std::vector<int> btree_sorted;
      inorderBTreeTraversal(root, btree_sorted);
	  std::cout << "btree_sorted:\n";
	  for (int i = 0; i < btree_sorted.size(); i++)
	  {
		  std::cout << btree_sorted[i] << "\t";
	  }
