...
void obhod(node* p_tree, int level, int& max_level,int& sch,vector<int>& l, vector<int>& k)
{   
	...
	if (p_tree != NULL)
	{
		...
		if (p_tree->p_left != NULL)
		{
			obhod(p_tree->p_left,  level + 1, max_level, sch,l,k);
		}
		if (p_tree->p_right != NULL)
		{
			obhod(p_tree->p_right,  level + 1, max_level, sch,l,k);
		}
...
void output2(node* p_tree, int level)
{
	int i;
	int j;
	vector<int> l;
