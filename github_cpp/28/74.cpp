...
Splay(int (*cut)(const T&),SPLAY_TREE_NODE<T,TV>*& tree) const
{
    ...
    SPLAY_TREE_NODE<T,TV> *x=tree;int cut_x;
    for(;;){
        cut_x=cut(x->key);
        if(cut_x==0)break;                                                                          // success
        else if(cut_x<0){
            SPLAY_TREE_NODE<T,TV> *y=x->left;if(!y)break;                                           // trivial
            int cut_y=cut(y->key);
            if(cut_y==0){*right_left=x;right_left=&x->left;x=y;cut_x=cut_y;break;}                  // zig
            else if(cut_y<0){
                SPLAY_TREE_NODE<T,TV> *z=y->left;
                if(!z){*right_left=x;right_left=&x->left;x=y;cut_x=cut_y;break;}                    // zig
                else{x->left=y->right;y->right=x;*right_left=y;right_left=&y->left;x=z;}}           // zig-zig
            else{
                SPLAY_TREE_NODE<T,TV> *z=y->right;
                if(!z){*right_left=x;right_left=&x->left;x=y;cut_x=cut_y;break;}                    // zig
                else{*left_right=y;left_right=&y->right;*right_left=x;right_left=&x->left;x=z;}}}   // zig-zag
        else{
            SPLAY_TREE_NODE<T,TV> *y=x->right;if(!y)break;                                          // trivial
            int cut_y=cut(y->key);
            if(cut_y==0){*left_right=x;left_right=&x->right;x=y;cut_x=cut_y;break;}                 // zig
            else if(cut_y>0){
                SPLAY_TREE_NODE<T,TV> *z=y->right;
                if(!z){*left_right=x;left_right=&x->right;x=y;cut_x=cut_y;break;}                   // zig
                else{x->right=y->left;y->left=x;*left_right=y;left_right=&y->right;x=z;}}           // zig-zig
            else{
                SPLAY_TREE_NODE<T,TV> *z=y->left;
                if(!z){*left_right=x;left_right=&x->right;x=y;cut_x=cut_y;break;}                   // zig
                else{*right_left=y;right_left=&y->left;*left_right=x;left_right=&x->right;x=z;}}}}  // zig-zag
    *left_right=x->left;
    x->left=left;
    *right_left=x->right;
    x->right=right;
