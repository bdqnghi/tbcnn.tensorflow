    ...
    ui(new Ui::TowerOfHanoi)
{
    ...
    ui->spinBox->hide();
    ui->pushButton_4->hide(); // In hiding so that it can only appear when set disks is clicked(just like spinbox.)
...
void TowerOfHanoi::on_pushButton_4_clicked() //Set Button here is used to set the disks just after choosing a value on spinbox.
{
...
void TowerOfHanoi::on_action_Autoplay_triggered()
{
    ...
    ui->action_Undo->setDisabled(true); // done because showing unexpected behavior when clicked during autoplay.
}
