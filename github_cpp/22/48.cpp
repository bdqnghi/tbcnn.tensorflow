...
void max_heapify(int* arr, int last_index, int root)
{
    ...
    //First check whether max_property is fulfilled at max
    if(first_child > last_index) //root is a leaf
        return;

    if(second_child > last_index && arr[root] >= arr[first_child]) //only one leaf is attached to root and max_property is fulfilled
        return;

    if(arr[root] >= arr[first_child] && arr[root] >= arr[second_child] && second_child <= last_index) //root has two children and max_property is fulfilled
        return;
    else
    {
        int target_child;
        if(second_child > last_index) //root has one leaf
