vector<string> vertices{"Seattle", "San Francisco", "Los Angeles", "Denver", "Kansas City", "Chicago", "Boston", "New York", "Atlanta", "Miami", "Dallas", "Houston"};
...
void dijkstra(int s, vector<int> &dist) {
    dist.resize(size, 2e9);
    dist[s] = 0;
    
    set<pair<int, int> > q;
    rep(i, size) q.insert({dist[i], i});
    ...
    while (!q.empty()) {
        ...
        for (int to: g[from]) {
            if (dist[from] + w[from][to] < dist[to]) {
                q.erase(q.find({dist[to], to})); //q.erase({dist[to], to});
