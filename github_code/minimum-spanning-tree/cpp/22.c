// minimumSpanningTree.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	for (int nodeNumber = 10; nodeNumber <= 1000000; nodeNumber *= 10) {
		...
		for (int i = 0; i < nodeNumber*5; i++) {	// nodeNumber*2/5/10
		...
		int E = nodeNumber*2;		// nodeNumber*2/5/10
		...
		for (int i = 0; i < E; i++) {
			...
			while (src == dst)
				dst = uni(rng);
			//if (edgeExistsInGraph(src, dst, graphk))
				//--i;
			//else {
