/*Am comentat algoritmul de verificare al quickSort-ului si a quickSelect-ului. Acestea functioneaza corect.
Cazul defavorabil pentru QuickSort este atunci cand sirul este sortat crescator, sortat descrescator sau cand toate elementele sunt egale.
*/
...
int partition (int *a, int p, int r, int n)
{
	int i, j, x;
	x = a[r];
	//profiler.countOperation("QuickSort: Numar de asignari", n);
	//profiler.countOperation("QuickSort: Numar de asignari si comparatii", n);
	quick_a++;
	quick_c_a++;
	i = p - 1;
		for (j = p; j <= r - 1; j++)
		{
			//profiler.countOperation("QuickSort: Numar de comparatii", n);
			//profiler.countOperation("QuickSort: Numar de asignari si comparatii", n);
			quick_c++;
			quick_c_a++;
			if (a[j] <= x)
			{
				i++;
				swap(&a[i], &a[j]);
				//profiler.countOperation("QuickSort: Numar de asignari", n, 3);
				//profiler.countOperation("QuickSort: Numar de asignari si comparatii", n, 3);
				quick_a += 3;
				quick_c_a += 3;
			}
		}
...
int main() {
	...
	for (i = 100; i < 10000; i += 100) {
		...
		quickSort(v, 0, i - 1, i);
		heapSort(v1, i);
		//divide(heap_a, heap_c, quick_a, quick_c);
