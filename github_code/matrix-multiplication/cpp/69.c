...
void MatrixMultiplication::convertToZorder(unsigned int startIndexI, unsigned int startIndexJ, unsigned int size){
	...
	if(size < 4){
		...
		cilk_spawn convertToZorder(startIndexI, startIndexJ, size / 2);
		cilk_spawn convertToZorder(startIndexI, startIndexJ + (size / 2), size / 2);
		cilk_spawn convertToZorder(startIndexI  + (size / 2), startIndexJ,  size / 2);
		cilk_spawn convertToZorder(startIndexI + (size / 2), startIndexJ + (size / 2), size / 2);
		cilk_sync;
	}
}
...
void MatrixMultiplication::convertToRowMatrix(unsigned int startIndexK, unsigned int length){
		if(length < 4){
			...
			cilk_spawn convertToRowMatrix(startIndexK, length / 4);
			cilk_spawn convertToRowMatrix(startIndexK + (length / 4), length / 4);
			cilk_spawn convertToRowMatrix(startIndexK + ((length / 4) * 2), length / 4);
			cilk_spawn convertToRowMatrix(startIndexK + ((length / 4) * 3), length / 4);
			cilk_sync;
		}
}
