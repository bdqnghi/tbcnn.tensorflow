/*
	psudo code
	start with startNode, provided as argument
	traverseQue.push(startNode)
	while(traverseQue not empty)
		ptr = adjacency list head of the front node of the traverseQueue
		while(ptr not NULL)
			if(node not visited)
				visited = true
				traverseQue.push(node)  // enque
