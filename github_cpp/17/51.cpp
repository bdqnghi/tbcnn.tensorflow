...
// Construct T1
        ...
        for (int i = 2; i <= len; i++) {
            ...
            if (DEBUG) {
                ...
                cerr << endl;
                cerr << i << ".0 "
                     << "Phase " << i << " adding " << next << " starting with " << startj << endl;
    ...
    bool rc)
{
    ...
    if (isLeaf) {
        if (isLeaf != queryflankingbase) {
            ...
            if ((left_in_ref != page->begin || page->shadow_left == -1) && (right_in_ref != page->end || page->shadow_right == -1)) {
