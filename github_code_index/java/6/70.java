...
/**
 * Typical merge sort - sorts in O(n log n) time but uses additional O(n) storage. 
 */
public class MergeSort {
    ...
    public void mergeArrays(int[] arr, int low, int mid, int high) {
        ...
        while (i <= mid && j <= high) {
        	//Both arrays have elements remaining..
        	if (temp[j] < temp[i]) {
        		arr[k++] = temp[j++]; 
        	} else {
        		arr[k++] = temp[i++]; 
        ...
        if (i > mid) {
        	//first array is exhausted, copy the remaining second array
        	while (j <= high) {
        		arr[k++] = temp[j++];
        	}
        }
        ...
        if (j > high) {
        	//second array is exhausted, copy the remaining first array
        	while (i <= mid) {
        		arr[k++] = temp[i++];
        	}
        }
