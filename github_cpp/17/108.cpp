...
inline int lh3_addon_is_match(HashedSets *hs, int m, char *array)
{
	...
	if (key == 0 || key % m == 1 || key % m == (unsigned)(m - 1)) return 1; 
	if (hs->find(key, &sk)) {
		int j;
		char *t_array, head[2];
		for (j = 0; j < sk->n; ++j) {
...
void lh3_cons_clear_cal_flag(arbre *tree)
{
	...
	for (i = 0; i < n_edge; ++i) {
		...
		for (k = 0; k < tree->lh3_tree.ctree->n_leaf; ++k) 
			tmp->array[0][k] = tmp->array[1][k] = 0;
	}
}
static void insert_to_HashedSets(HashedSets *hs, int m, char *array) 
{
	SameKey *sk;
...
HashedSets *lh3_addon_cal_HashedSets(Tree *tree) 
{
	Tree **nodes;
	int i, n_leaf, m;
