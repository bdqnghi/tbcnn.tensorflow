...
public class FordFulk {
	...
	public void fordFulkerson() {
		...
		rg.clearFlow();
		while(rg.findAugmentingPath() != null){
			LinkedList<Edge> edgeList = rg.findAugmentingPath();
			int increaseFlow = Integer.MAX_VALUE;
			int[][] res_capacity = rg.getRes_capacity();
			for(int i=0;i<edgeList.size();i++){
				int c = res_capacity[edgeList.get(i).getSourceVertex().getLabel()][edgeList.get(i).getTargetVertex().getLabel()];
				if(c < increaseFlow){
					increaseFlow = c;
				}
			}
			for(Edge e : edgeList){
				rg.increaseFlow(increaseFlow, e);
				int source = e.getSourceVertex().getLabel();
				int target = e.getTargetVertex().getLabel();
				rg.setRes_capacity(source,target,res_capacity[source][target] - increaseFlow);
				rg.setRes_capacity(target, source, res_capacity[target][source] + increaseFlow);
			}
		}
	}
