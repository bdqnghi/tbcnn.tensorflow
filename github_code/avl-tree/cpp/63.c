...
int getBalanceFactor(Node* node) { // heightOfLeftChild - heightOfRightChild
    if (!node) return 0;
...
Node* insert(int key, Node* root) {
    ...
    if (2 <= balanceFactor && key < root->left->key) { // Left Heavy Tree
                                                       // Rotate to right (RR)
        return rotateRight(root);
    } else if (2 <= balanceFactor && key > root->left->key) { // Left Heavy Tree
                                                              // Rotate Left then Rotate right
        root->left = rotateLeft(root->left);
        return rotateRight(root);

    } else if (-2 >= balanceFactor && key > root->right->key) { // Right Heavy Tree
                                                                // Rotate Left (LL)
        return rotateLeft(root);
    } else if (-2 >= balanceFactor && key < root->right->key) { // Right HeavyTree
...
Node* deleteNode(int key, Node* root) {
    ...
    if (2 <= balanceFactor && leftBalanceFactor >= 0) { // Left Heavy Tree
                                                        // Rotate to right (RR)
        return rotateRight(root);
    } else if (2 <= balanceFactor && leftBalanceFactor < 0) { // Left Heavy Tree
                                                              // Rotate Left then Rotate right
        root->left = rotateLeft(root->left);
        return rotateRight(root);
    } else if (-2 >= balanceFactor && rightBalanceFactor <= 0) { // Right Heavy Tree
                                                                 // Rotate Left (LL)
        return rotateLeft(root);
    } else if (-2 >= balanceFactor && rightBalanceFactor > 0) { // Right HeavyTree
                                                                // Rotate Right then Rotate Left
        root->right = rotateRight(root->right);
        return rotateLeft(root);
    }
