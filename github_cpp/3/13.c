...
int avlTree::bfactor(CNode* p) 
{
	return height(p->right) - height(p->left);
}

void avlTree::fixheight(CNode* p) 
{
...
CNode* avlTree::rotateright(CNode* p) 
{
	CNode* q = p->left;
	p->left = q->right;
	q->right = p;
...
CNode* avlTree::rotateleft(CNode* q) 
{
	CNode* p = q->right;
	q->right = p->left;
	p->left = q;
...
bool avlTree::add(int k) { 
	root = insert(root, k);
	return true;
}

int avlTree::findplace(CNode* p, int k) 
{
	if (p == nullptr) {
		return 0;
	}
...
CNode* avlTree::insert(CNode* p, int k) 
{
	if (p == nullptr) {
...
std::pair<CNode*, CNode*> avlTree::removemin(CNode* p) 
{
	if (p->left == 0)
...
CNode* avlTree::remove(CNode* p, int k) 
{
...
void avlTree::printAsTree(CNode* node, int currentLevel) { 
	if (node == 0) {
		return;
	}
