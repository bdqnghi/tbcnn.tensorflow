...
int m[50][50]; //matricea ponderilor
int n; //numarul varfurilor i.e. al oraselor pe harta
...
Alegerea dumneavoastra: ";
    ...
    //Importarea hartii i.e. citirea graficului
    switch(switch_mode)
    {
        case 1: //ROMANIA
        {
...
Alegerea dumneavoastra: ";
            ...
            switch(switch_mode_RO)
            {
                case 1: //De la tastatura
                {
                    ...
                    int counter; //numara randurile de muchii cu costuri
                    int x, y, z; //pentru citirea fiecarui rand

                    RO>>counter;
                    for(int i = 1; i <= counter; i++)
                    {
                        RO>>x; //citire primul nod
                        RO>>y; //citire al doilea nod
                        RO>>z; //citire cost

                        m[x][y] = m[y][x] = z; //initializare matrice
                    }

                    int num_o, locations[n+2];
                    //num_o: numar de comenzi
                    //locations: vector care indica numarul de camioane in fiecare oras


                    int num_trucks = 0; //numarul de camioane
                    int num_locs = 0; //numarul de locatii ocupate de camioane
                    ...
                    //Matricea comenzilor: liniile 0 pentru sursa
                    //liniile 1 pentru destinatie
                    //liniile 2 pentru locatia camionului atasat
                    ...
                    for(int i = 1; i <= num_o; i++)
                    {
                        ...
                        orders[0][i] = x; //sursa
                        orders[1][i] = y; //destinatie
                        orders[2][i] = -1; //locatie camion
                    }
                    ...
                    //Se va actualiza matricea ponderilor cu cele mai scurte drumuri
                    //de la oricare oras i la oricare oras j
                    royfloyd();
                    ...
                    if(num_trucks >= num_o)
                    {
                        ...
                        //Matricea distances memoreaza:
                        //pe coloanele 0 distantele
                        //pe coloanele 1 sursa comenzii
                        //pe coloanele 2 numarul de ordine al sursei comenzii
                        //pe coloanele 3 locatia camionului
                        for(int i = 1; i <= num_o; i++)
                            for(int j = 1; j <= n; j++)
                                if(locations[j])
                                {
                        ...
                        //Vom alege cele mai optime asocieri camion-comanda
                        //Odata ce un camion a fost asociat unei comenzi, el va fi considerat blocat
                        //In acest sens, vom decrementa numarul de camioane din locatia respectiva
                        int check = 0;
                        for(int i = 1; i <= k; i++)
                        {
                        ...
                        //In acest moment se cunoaste ce camion i se asociaza fiecarei comenzi
                        //Vom afisa de acum incolo drumul parcurs de camioane pentru a rezolva
                        //comenzile
                    ...
                    {
                        //Numarul de repetari ale algoritmului
                        ...
                        //Vectorul declarat mai sus retine comenzile efectuate
                        //intr-o anumita iteratie a for-ului care urmeaza

                        for(int current_iteration = 1; current_iteration <= times; current_iteration++)
                        {
                            //Initializare distante curente dupa cea mai recenta livrare
                            k = 0;
                            for(int i = 1; i <= num_o; i++)
                            {
                                if(orders[2][i])
                            ...
                            //Variabila care regleaza numarul de camioane trimise la comenzi
                            int assigned_trucks;
                            ...
                            for(int i = 1; i <= assigned_trucks; i++)
                            {
                                ...
                                //Fiecare camion este replasat pentru urmatoarea iteratie:
                                locations[orders[1][list_of_orders[i]]]++;
                                //Sunt marcate comenzile efectuate:
...
Alegerea dumneavoastra: ";
            ...
            switch(switch_mode_GER)
            {
                case 1: //De la tastatura
                {
                    ...
                    for(int i = 1; i <= counter; i++)
                    {
                        GER>>x; //citire primul nod
                        GER>>y; //citire al doilea nod
                        GER>>z; //citire cost
