...
public class FordFulkerson {
     ...
     */
    public FordFulkerson(FlowNetwork G, int s, int t, boolean maxFlow, double val) {
        value = val;
        validate(s, G.V());
        validate(t, G.V());
        ...
        boolean feasible = isFeasible(G, s, t);
        if (!feasible) throw new IllegalArgumentException("Initial flow is infeasible");
        ...
        if(maxFlow) {
            ...
            value = excess(G, t);
            while (hasDecreasingPath(G, s, t)) {

                // compute bottleneck capacity
                double bottle = Double.POSITIVE_INFINITY;
                for (int v = t; v != s; v = edgeTo[v].other(v)) {
                    bottle = Math.min(bottle, edgeTo[v].residual());
                }

                // augment flow
                for (int v = t; v != s; v = edgeTo[v].other(v)) {
                    edgeTo[v].decreaseResidualFlowTo(v, bottle); 
                }
...
//            assert check(G, s, t);
    ...
    private boolean hasDecreasingPath(FlowNetwork G, int s, int t) {
        edgeTo = new FlowEdge[G.V()];
        marked = new boolean[G.V()];
        ...
        while (!queue.isEmpty() && !marked[t]) {
            ...
            for (FlowEdge e : G.adj(v)) {
                int w = e.other(v);
                FlowEdge fe = G.adjTo(w, v);
                double residual = fe.capacity()-fe.flow()+e.flow()-e.lowerBound();
                // if residual capacity from v to w
                if (residual > 0) {
                    if (!marked[w]) {
                        e.residual(residual);
