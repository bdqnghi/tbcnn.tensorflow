...
int bottom_up()
{
    ...
    /// empty source k , destination string er 'first j char.' porjonto transfer korte minimum operation .. " (INSERTION) "
    for(int j=1; j<dl; j++) dp[0][j] = j;   // jeta 0 , seta empty string mean kore ...
    /// non-empty source k , empty destination string a transfer korte minimum operation ... " (DELETION) "
    ...
    for(int i=1;i<=sl; i++)
    {
        for(int j=1; j<=dl; j++)
        {
            if(SS[i-1] == DS[j-1]) dp[i][j] = dp[i-1][j-1]; /// no operation required as characters are the same

            else dp[i][j] = 1 + min(dp[i-1][j-1],       /// substitution (replace/swap)
