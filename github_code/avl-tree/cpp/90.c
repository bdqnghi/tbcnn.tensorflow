...
int AvlTree::int_path_length(AvlNode *t, int depth) {
  ...
  if (t->left == NULL) return depth + int_path_length(t->right, depth+1);
  if (t->right == NULL) return depth + int_path_length(t->left, depth+1);
  else return depth + int_path_length(t->left, depth + 1) + int_path_length(t->right, depth + 1);
