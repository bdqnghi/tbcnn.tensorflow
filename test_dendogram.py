...
def generate_trees_from_cluster(left_trees,right_cluster):
    temp_left_trees = []
    temp_right_trees = []
    for left_tree in left_trees:
        for right_tree in right_cluster:
            temp_left_trees.append(left_tree)
            temp_right_trees.append(right_tree)

    return temp_left_trees, temp_right_trees


def test_model(logdir, targets, clusters, left_embedfile, right_embedfile,target,epochs=EPOCHS):
    ...
    with open(targets, "rb") as fh:
        target_algorithm = pickle.load(fh)


    with open(clusters, "rb") as fh:
    ...
    for cluster in clusters:
        left_trees, right_trees = generate_trees_from_cluster(target_algorithm, cluster)
        print "------------------------------------------------------------"
        name = "Left programs : " + str(left_trees[0]["label"]) + " vs " + "Right cluster : " + str(cluster[0]["label"]) 
        # print "Left label : " + str(left_trees[0]["label"]) 
        # print "Right label : " + str(cluster[0]["label"]) 
   

        print name
        print('Computing testing accuracy...')

        for left_gen_batch, right_gen_batch in sampling.batch_random_samples_2_sides(left_trees, left_algo_labels, right_trees, right_algo_labels, left_embeddings, left_embed_lookup, right_embeddings, right_embed_lookup, using_vector_lookup_left, False, 100):
            ...
            list_prob = []
            # print output
            for softmax_prob in output[0]:

                list_prob.append(softmax_prob[1])
            print "------------------"
            print list_prob
            print np.mean(np.array(list_prob))
            with open("./dendogram/" + target + ".txt", 'a') as file_handler:
                file_handler.write("-------------------------------\n" )
                file_handler.write(name + "\n")
                file_handler.write(str(np.mean(np.array(list_prob))) + "\n")
                # file_handler.write(name)
...
def main():
    ...
    test_model(sys.argv[1],sys.argv[2],sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
