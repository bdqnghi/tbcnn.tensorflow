 ...
 * 2、将频率转换为索引  即累计更新过程   即累计值+当前count值即更新为当前accum累计的索引值   索引为已排序结果在数组中的索引
 * 3、数据分类排序
 * @author wjs13
 *可用于 字符串排序  因为字符串的每一个字符的取值范围很小  固可基于 桶/计数排序
 */
...
//          则累计数组accum数组 既可以表示索引数组 即key为i的键在有序序列中索引为[accum[i],accum[i+1])
       ...
       for (int i = 0; i < n; i++){//分类 按 桶中频率的计数值 和 由计数值转换为索引  来 排序
           aux[accum[a[i].key]++] = a[i];//每分类排序一个  则对应下一个元素的索引就要自增1
       }
       for (int i = 0; i < n; i++){
           a[i] = aux[i];//回写排序结果
       }
